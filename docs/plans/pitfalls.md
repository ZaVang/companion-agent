...
### [代码质量] MemorySystem不能无限膨胀
- **错误做法**: 所有功能都塞进一个类
- **正确做法**: 使用Facade模式，类只负责协调，具体逻辑委托给子模块
- **原因**: 400+行的类难以维护、测试、理解

### [代码质量] 文档不能碎片化
- **错误做法**: 每个Sprint都新建目录，到处散落
- **正确做法**: 统一到chronicle.md，按时间线组织
- **原因**: 碎片化导致信息丢失，新人难以查找

### [代码质量] 类型提示不能省略
- **错误做法**: helper函数不写类型，靠"能跑就行"
- **正确做法**: 所有公开函数必须有类型提示和docstring
- **原因**: 没有类型提示的代码是技术债，IDE无法提供帮助

### [代码质量] 测试必须先于重构
- **错误做法**: 重构后再补测试
- **正确做法**: 重构前确保测试全部通过，重构后再次验证
- **原因**: 没有测试的重构是盲目重构